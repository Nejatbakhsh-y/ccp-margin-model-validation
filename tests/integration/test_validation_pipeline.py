...
def test_validation_result_rows_support_exception_and_shortfall_recalculation():
    ...
    margin_col = next(
        (column for column in margin_candidates if column in frame.columns), None
    )
    loss_col = next(
        (column for column in loss_candidates if column in frame.columns), None
    )

    assert margin_col is not None, "Validation results lack a recognized margin field."
    assert loss_col is not None, (
        "Validation results lack a recognized realized-loss field."
    )
    ...
    if "exception_flag" in frame.columns:
        ...
        if actual_exception.dtype != bool:
            actual_exception = (
                actual_exception.astype(str)
                .str.strip()
                .str.lower()
                .isin({"1", "true", "yes", "y", "exception", "breach"})
            )
        np.testing.assert_array_equal(
            actual_exception.to_numpy(), expected_exception.to_numpy()
        )
